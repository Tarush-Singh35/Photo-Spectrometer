...
'figure_name','Arduino and scilab. Measuring the spectrum of a light'); // Gui window
...
'SliderStep',[0.01,0.1],'String','Start measuring spectrum','Style','pushbutton','Value',[0],'VerticalAlignment','middle','Visible','on','Tag', ...
...
'SliderStep',[0.01,0.1],'String','Stop measuring spectrum','Style','pushbutton','Value',[0],'VerticalAlignment','middle','Visible','on','Tag', ...
...
Measurment_points =100; // Measurment that are plotted.
Ymin = 1; // Y axes minimun.
Ymax =7; // Y axes maximun.
...
a.title.text="Spectrum" // Setting the graph name.
...
Serial_communication = openserial(3,"9600,n,8,1") // Open usb serial communication using default values.i.e 9600 baud rate no parity with 8 character.
...
Serial_data = readserial(Serial_communication,1); // Read a character from the usb buffer.
   ...
   if ascii(Serial_data)== 13 then
    ...
    Serial_data_2 = readserial(Serial_communication,7); // Read the string from the Arduino usb send.

    Measurment = strtod(Serial_data_2); // Convert string to double.

    //disp(Measurment); // Output to scilab console.

    Box_text = 'Current Intensity : '+string(Measurment)+" V "; // Measurment display box show Current Intensity.
    set(handles.Out_text,'String',Box_text); // // Measurment display box show Current Intensity.

    e = findobj("tag", "real-time plot"); // Prepare axes for the real time plot.

    // Passing the measurment spectrum to the real time plot.
